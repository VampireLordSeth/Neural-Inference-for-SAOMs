...
| **M4 — Application** | Many networks at once, at a scale existing tools cannot reach. Step 1: the estimator extends to networks of up to 200 actors, calibrated at every size; on the full Glasgow school panel (129 pupils) it matches RSiena on the structural effects and reads the change rates low, which traces to the training population being too dense for a real school network — the fix for step 2. | in progress · `docs/M4_RESULTS.md` |
